...
class RAM {
  ...
  write(address, value) {
    this.memory[address] = value.toString(2);
    this.ramInfo.memory.content = this.memory;
  }

  writeToRAM(address, value) {
    this.memory[address] = String(value).charCodeAt(0).toString(2);
    this.ramInfo.memory.content = this.memory;
  }

  ramToSymb(address) {
    return String.fromCharCode(parseInt(this.memory[address].toString(), 2))
  }

  loadToRAM(string) {
    for (let i = 0; i < String(string).length; i++) {
      this.write(i, String(string).charCodeAt(i));
    }
    this.ramInfo.memory.content = this.memory;
  }
  ...
  readAllFromRAM() {
    ...
    const resultArray = [];
    let rs = "";
    for (let y = 0; y < this.memory.length; y++) {
      if (this.memory[y] != 0) {
        startArray.push(this.memory[y]);
      }
    }
    for (let i = 0; i < startArray.length; i += batchSize) {
      const batch = startArray.slice(i, i + batchSize);
      const processedBatch = batch.map(item => {
        return parseInt(item.toString(), 2)
      });
      resultArray.push(...processedBatch);
    }
    for (let i = 0; i < resultArray.length; i += batchSize) {
      ...
      rs += processedBatch.join("");
    }
    let end = performance.now();
    console.log(end - start)
    // const codeString = String.fromCharCode(...(this.memory.map(num => parseInt(num.toString(), 2))));
    return rs;
  }
...
const RAMWorker = new RAM(1024 * 1024 * 100)//100 MB
// RAMWorker.loadToRAM(`Устная передача — самый древний способ передачи знаний в истории человечества. После изобретения древними цивилизациями систем записи люди начали использовать для письма почти всё, на чём можно писать — глиняные таблички, кору дерева, листы металла и т. п.
// Таблички
...
// `)
// RAMWorker.write(0x0, 101)
// // console.log(RAMWorker.memory)
// // console.log(RAMWorker.ramToSymb(0x0)
// console.log(RAMWorker.readAllFromRAM())
// console.log(RAMWorker.ramInfo)
